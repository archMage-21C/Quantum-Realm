...
async function getEntriesFromSearch(html3) {
  const $2 = load(html3);
  const entryPromises = $2('a[href^="/word/"]:not(.crossreference):not(.link)').map(async (_i, el) => {
    const $el = $2(el);
    const term = $el.find("span").first().text();
    const path = $el.attr("href");
    const termHtml = await getHtml(BASE_URL + path);
    const $term = load(termHtml);
    const def = $term("section.-mt-4.-mb-2.lg\\:-mb-2 > p").map((_i2, el2) => $term(el2).text()).get().join("\n\n").trim().replace(/\t/g, "");
    const id = getIdFromPath(path);
    return { term, def, path, id };
  }).get();
...
function displayEntries(entries, contentEl, searchTerm) {
  if (!entries.length) {
